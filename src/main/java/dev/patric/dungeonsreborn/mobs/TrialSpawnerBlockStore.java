...
public final class TrialSpawnerBlockStore {
  public record Entry(String trialSpawnerId, String world, int x, int y, int z, String ownerId, long createdAtMillis) {
  }
  ...
  public TrialSpawnerBlockStore(File dataFolder, ServiceLogger logger) {
    Objects.requireNonNull(dataFolder, "dataFolder");
    this.file = new File(dataFolder, "trial_spawner_blocks.yml");
  ...
  public void load() {
    ...
    for (String key : sec.getKeys(false)) {
      ...
      String trialSpawnerId = node.getString("trialSpawnerId", null);
  ...
  public void save() {
    ...
    for (Entry entry : entries.values()) {
      ...
      node.set("trialSpawnerId", entry.trialSpawnerId());
    ...
    try {
      ...
      logger.warn("[Mobs] Failed to save trial_spawner_blocks.yml: " + ex.getMessage());
    }
  }

  public void upsert(Block block, String trialSpawnerId, String ownerId) {
    if (block == null || block.getWorld() == null || trialSpawnerId == null || trialSpawnerId.isBlank()) {
      return;
    }
  ...
  public int rehydrateMarkers() {
    ...
    for (Entry entry : entries.values()) {
      ...
      MobSpawnerMarkers.setTrialSpawnerId(block, entry.trialSpawnerId());
      if (entry.ownerId() != null) {
        try {
          MobSpawnerMarkers.setTrialSpawnerOwner(block, java.util.UUID.fromString(entry.ownerId()));
