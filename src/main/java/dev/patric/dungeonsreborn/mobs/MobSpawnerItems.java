...
public final class MobSpawnerItems {
  ...
  public static ItemStack decorateSpawnerItem(ItemStack base, MobSpawnerBlockSpec spec, MobSpec mobSpec) {
    ...
    meta.displayName(MobText.parse("<gold>" + mobLabel + " Spawner</gold>"));
    List<Component> lore = new ArrayList<>();
    lore.add(MobText.parse("<gray>Mob:</gray> <white>" + mobLabel + "</white>"));
    lore.add(MobText.parse("<dark_gray>Id:</dark_gray> <gray>" + spec.mobId() + "</gray>"));
    MobSpawnerTemplate template = spec.template();
    if (template != null) {
      if (template.maxAlive() != null) {
        lore.add(MobText.parse("<gray>Max Alive:</gray> <white>" + template.maxAlive() + "</white>"));
      }
      if (template.respawnTicks() != null) {
        long seconds = Math.max(0L, template.respawnTicks()) / 20L;
        lore.add(MobText.parse("<gray>Respawn:</gray> <white>" + seconds + "s</white>"));
      }
      if (template.radius() != null) {
        lore.add(MobText.parse("<gray>Spawn Radius:</gray> <white>" + template.radius() + "</white>"));
      }
      if (template.attackRadius() != null) {
        lore.add(MobText.parse("<gray>Attack Radius:</gray> <white>" + template.attackRadius() + "</white>"));
      }
    }
    lore.add(MobText.parse("<dark_gray>Place to create a spawner.</dark_gray>"));
    meta.lore(lore);
